...
int main() {
  ...
  if (!client) {
    fprintf(stderr, "creation of new mosquitto instance failed\n");
  }
  ...
  // connect to broker
  printf(
      "connecting to test.mosquitto.org on port:1883 (unauth, unencrypted)\n");
  if (mosquitto_connect(client, "test/f1/telemety", 1883, 30) !=
      MOSQ_ERR_SUCCESS) {
